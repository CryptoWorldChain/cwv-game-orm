...
public interface CWVGameMapMapper extends StaticTableDaoSupport<CWVGameMap, CWVGameMapExample, CWVGameMapKey> {
     ...
     */
    @Insert({
        "insert into cwv_game_map (map_id, game_city_id, ",
        "map_name, image_url, ",
        "created_time, status, ",
        "is_display)",
        "values (#{mapId,jdbcType=INTEGER}, #{gameCityId,jdbcType=INTEGER}, ",
        "#{mapName,jdbcType=VARCHAR}, #{imageUrl,jdbcType=VARCHAR}, ",
    ...
    @Select({
        "select",
        "map_id, game_city_id, map_name, image_url, created_time, status, is_display",
        "from cwv_game_map",
        "where map_id = #{mapId,jdbcType=INTEGER}"
    })
    @Results({
