...
public interface CWVGameCityMapper extends StaticTableDaoSupport<CWVGameCity, CWVGameCityExample, CWVGameCityKey> {
     ...
     */
    @Insert({
        "insert into cwv_game_city (city_id, game_country_id, ",
        "city_name, status, ",
        "is_display, created_time)",
        "values (#{cityId,jdbcType=INTEGER}, #{gameCountryId,jdbcType=INTEGER}, ",
        "#{cityName,jdbcType=VARCHAR}, #{status,jdbcType=VARCHAR}, ",
    ...
    @Select({
        "select",
        "city_id, game_country_id, city_name, status, is_display, created_time",
        "from cwv_game_city",
        "where city_id = #{cityId,jdbcType=INTEGER}"
    })
    @Results({
